...
const Exam = observer(() => {
    ...
    useEffect(() => {
        ...
        if (cesObj.error) {
            if (cesObj.error.networkError?.message.split("<")[0].trim() == "Unexpected token") {
                localStorage.removeItem("accessToken");
                navigate("/")
            }
        }
    }, [cesObj.data, cesObj.error])
                        ...
                        <Grid container justifyContent="flex-start" alignContent="flex-start" alignItems="flex-start" spacing={2} >
                            ...
                            <Grid item>
                                {questionsArray.length > 0
                                    ...
                                    : <Button variant="outlined" size="large" onClick={getResult} disabled style={{color:"gray",border:"1px solid gray"}}>Get Result</Button>
                                }
                            </Grid>
